 ...
 */
class TeTypeLoader implements ExecutableTestSuiteLoader {
    ...
    private static String notice(final String apiUrl) {
        return "<br/><br/> "
                + "This Executable Test Suite is executed using a "
                + "<a target=\"_blank\" href=\"" + apiUrl + "\">remote TEAM Engine instance</a>"
                + (apiUrl.contains("opengeospatial.org/") ? " hosted by OGC for their Compliance Program (CITE)." : ".")
                + " The results are transformed into the ETF internal report format. Some information that is "
    ...
    private final EidSet<ExecutableTestSuiteDto> executableTestSuites = new DefaultEidSet<>();
    ...
    private void updateEts(final ExecutableTestSuiteDto ets) throws ObjectWithIdNotFoundException, StorageException {
        ((WriteDao) etsDao).replace(ets);
        this.loadingContext.getItemRegistry().update(executableTestSuites);
    }

    @Override
    public void release() {
        this.loadingContext.getItemRegistry().deregister(executableTestSuites);
    }

    @Override
    public EidSet<ExecutableTestSuiteDto> getExecutableTestSuites() {
        return this.executableTestSuites;
    }
        ...
        for (final ExecutableTestSuiteDto ets : eTestSuitesToAdd) {
            try {
                if (!etsDao.exists(ets.getId()) || etsDao.isDisabled(ets.getId())) {
                    ...
                    this.loadingContext.getItemRegistry().register(executableTestSuites);
                }
        ...
        try {
            ...
            for (final Element etsUrl : etsUrls) {
                ...
                if (create) {
                    ...
                    ets.setDescription(description + notice(apiUri.toString()));
            ...
            return executableTestSuites.toList();
